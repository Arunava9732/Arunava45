...
router.post('/newsletter/subscribe', (req, res) => {
  try {
    ...
    // Initialize subscribers array if not exists
...
router.get('/newsletter/subscribers', authenticate, requireAdmin, (req, res) => {
  try {
    const data = readData();
...
router.post('/newsletter/unsubscribe', (req, res) => {
  try {
    ...
    const idx = data.newsletterSubscribers.findIndex(
      s => s.email.toLowerCase() === email.trim().toLowerCase()
    );
    ...
    data.newsletterSubscribers[idx].active = false;
    data.newsletterSubscribers[idx].unsubscribedAt = new Date().toISOString();
...
router.delete('/newsletter/subscribers/:id', authenticate, requireAdmin, (req, res) => {
  try {
    const data = readData();
    ...
    const idx = data.newsletterSubscribers.findIndex(s => s.id === req.params.id);
    
    if (idx === -1) {
      return res.status(404).json({ success: false, error: 'Subscriber not found' });
    }
    
    data.newsletterSubscribers.splice(idx, 1);
    writeData(data);
