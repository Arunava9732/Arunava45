...
const sendEmail = async (options) => {
  ...
  try {
    logger.info(`📧 [Email ${emailId}] Sending to ${options.to}: ${options.subject}`);
    ...
    if (info.simulated) {
      logger.warn(`📧 [Email ${emailId}] SIMULATED (SMTP not configured) - would have sent to ${options.to}`);
      return info;
    }
    
    logger.info(`📧 [Email ${emailId}] ✅ Sent successfully: ${info.messageId}`);
    aiEmailLog('SENT', { emailId, messageId: info.messageId, to: options.to });
    return info;
  } catch (error) {
    logger.error(`📧 [Email ${emailId}] ❌ FAILED: ${error.message}`);
    logger.error(`📧 [Email ${emailId}] Stack: ${error.stack}`);
    aiEmailLog('FAILED', { emailId, error: error.message, to: options.to });
    
    // Log specific SMTP errors for debugging
    if (error.code) {
      logger.error(`📧 [Email ${emailId}] Error code: ${error.code}`);
    }
    if (error.response) {
      logger.error(`📧 [Email ${emailId}] SMTP Response: ${error.response}`);
    }
...
const sendGiftCardEmail = async (giftCard, recipientEmail, senderName) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px; border-top: 5px solid #000;">
      ...
      <div style="background: #f9f9f9; padding: 20px; border-radius: 8px; text-align: center; margin-bottom: 20px;">
        ...
        <p style="font-size: 24px; color: #059669; font-weight: bold;">Value: ₹${giftCard.amount}</p>
  ...
  return sendEmail({
    ...
    subject,
    html,
    text: `You've received a BLACKONN Gift Card for ₹${giftCard.amount}! Code: ${giftCard.code}. Use it at blackonn.com`
  });
};
