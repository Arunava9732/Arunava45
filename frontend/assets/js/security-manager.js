...
class SecurityManager {
  ...
  setupCSP() {
    ...
    meta.content = [
      ...
      "connect-src 'self' ws: wss: https: https://www.googleapis.com",
      "base-uri 'self'",
      "form-action 'self'"
    ].join('; ');
  ...
  monitorConsole() {
    ...
    console.log = function(...args) {
      if (args.some(arg => typeof arg === 'string' && /password|token|secret/i.test(arg))) {
        SecurityManager.instance.detectThreat('Console', 'Sensitive data logged to console', 'medium');
      }
  ...
  monitorNetworkRequests() {
    ...
    window.fetch = async function(...args) {
      ...
      // Rate limit check
      if (!SecurityManager.instance.checkRateLimit(url)) {
