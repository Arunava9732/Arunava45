...
class RealtimeManager {
   ...
   */
  handleError(event) {
    // WebSocket connection errors are common and expected in some environments
    // We log them as warnings to prevent flooding error trackers
    console.warn(`❌ WebSocket error${this.reconnectAttempts > 0 ? ` (attempt ${this.reconnectAttempts})` : ''}:`, 
    ...
    if (typeof AIAnalytics !== 'undefined' && typeof AIAnalytics.track === 'function') {
      AIAnalytics.track('realtime_error', { type: 'connection_error' });
    }
  }
